...
5. 给每个域名单独配置对应的 Stripe 名称策略
6. 给每个域名单独配置对应的 Stripe 密钥
...
每个落地域名还需要单独选择一个 Stripe 商品名称展示策略：
...
- 这个策略只影响 Stripe Hosted Checkout 页面上的商品名称展示
- 不影响实际扣款金额
- `来源商品真实名称` 当前取 intake 商品列表中第一个非空商品名
...
8. 添加域名池
9. 为每个域名设置模板
10. 为每个域名设置 Stripe 名称策略
11. 为每个域名设置 Stripe 密钥
12. 创建分销商后台账号
