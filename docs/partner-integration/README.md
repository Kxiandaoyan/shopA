...
- 代理商买家不会在我方站内看到独立收银台表单
- 落地页仅用于完成域名校验与风控要求，会在极短时间内自动进入 Stripe Hosted Checkout
- 支付结果仍然会先落我方站点，再回跳代理商，确保本地订单状态、日志和异步通知先完成
...
- Stripe Hosted Checkout 页面上的商品名称由平台按落地域名策略控制，不一定等于代理商传入的 `items.name`
- 若域名策略为“来源商品真实名称”，当前默认取 `items` 中第一条非空商品名
...
### 15.1 为什么不能直接跳 Stripe？

因为本平台业务要求必须先进入我方首页，再由首页自动跳到支付页。这是固定流程，不支持代理商直接拼 Stripe 地址。

### 15.2 为什么支付成功后不直接从 Stripe 回代理商？

因为系统需要先在我方站点记录订单结果、写日志并完成状态同步，然后才回跳到代理商。

### 15.3 `returnUrl` 为什么要提前登记？

为了避免任意回跳地址带来的安全风险，所以只允许已登记的地址。

### 15.4 没有配置 `callbackSecret` 能否接入？

可以。系统仍会回跳，但回跳参数无法做到强校验。正式生产环境建议一定配置 `callbackSecret`。

### 15.5 商品价格为什么以接口传入金额为准？

因为本平台支持代理商侧自定义订单金额，支付价格以下单接口中的 `totalAmount` 为准。

### 15.6 Stripe 页面上的商品名称为什么可能和代理商传入名称不同？

因为平台支持按落地域名单独配置 Stripe 商品名称展示策略，用于统一收银页展示口径。  
这不会影响最终支付金额，也不会影响回跳和 webhook 字段。

### 15.7 现在是否有独立的服务端回调通知？
