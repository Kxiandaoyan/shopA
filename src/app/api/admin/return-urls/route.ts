...
import { returnUrlAdminSchema } from "@/lib/admin/schemas";
import { db } from "@/lib/db";
import { writeAuditLog } from "@/lib/logging/audit";

const returnUrlUpdateSchema = returnUrlAdminSchema.extend({
  id: z.string().min(1),
});
...
export async function POST(request: Request) {
  ...
  const parsed = returnUrlAdminSchema.safeParse(body);
  ...
  const entry = await db.affiliateReturnUrl.upsert({
    where: {
      affiliateId_url: {
  ...
  await writeAuditLog({
    ...
    eventType: "admin.return_url_upserted",
    result: LogResult.SUCCESS,
    targetType: "affiliate_return_url",
    targetId: entry.id,
...
export async function PATCH(request: Request) {
  ...
  const body = await request.json();
  const parsed = returnUrlUpdateSchema.safeParse(body);
  ...
  const entry = await db.affiliateReturnUrl.update({
  ...
  await writeAuditLog({
    actorId: auth.session.sub,
    eventType: "admin.return_url_updated",
    result: LogResult.SUCCESS,
    targetType: "affiliate_return_url",
    targetId: entry.id,
    metadata: parsed.data,
